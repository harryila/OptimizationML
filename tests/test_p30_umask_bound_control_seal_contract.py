...
def _runbook_array_names(runbook: str, array_name: str) -> list[str]:
    marker = f"{array_name}=(\n"
    assert runbook.count(marker) == 1
    body = runbook.split(marker, 1)[1].split("\n)", 1)[0]
    names: list[str] = []
...
def test_preexecution_revision_orders_receipt_review_before_seal() -> None:
    ...
    freeze_source = (
        "freeze the complete P30 source package as one direct child of the terminal P29 outcome"
    )
    frozen_client_tools = (
        "materialize the client logger preflight receipt-reviewer local-bundle-verifier "
        "authenticated-transfer and remote-bundle-verifier blobs from the exact source "
        "commit into external O_EXCL "
        "mode-0400 files, verify their frozen SHA-256 values, and pass the credential-free "
        "read-only actual-host prerequisite preflight"
    )
    provision = (
        "provision the previously absent root-owned P30 directory sole process-owned "
        "transport capability root and separate empty root-owned localization ledger root "
        "exactly once"
    )
    create_runtime_receipt = (
        "create and locally verify the exact phase-specific head and independently close a "
        "second full nine-ref runtime-review bundle in a second fresh empty bare repository"
    )
    review_runtime_receipt = (
        "publish the external runtime-review receipt and independently review every frozen "
        "namespace attempt snapshot and artifact authority field before localization dispatch"
    )
    localization_dispatch = next(
        item for item in order if item.startswith("at run-localization dispatcher entry")
    )

    assert order.index(write_receipt) < order.index(review_receipt) < order.index(seal_source)
    assert order.index(freeze_source) < order.index(frozen_client_tools) < order.index(provision)
    assert (
        order.index(create_runtime_receipt)
        < order.index(review_runtime_receipt)
        < order.index(localization_dispatch)
    )
    ...
    assert freeze["phase_one_historical_v5_commit"] == ("8286d45db45c27e219724cfb9ace2108a2823301")
    assert freeze["phase_one_historical_v5_annotated_tag_object"] == (
    ...
    assert freeze["phase_one_historical_v5_tag_object_and_peel_are_unchanged_after_amend"]
    assert freeze[
    ...
    assert freeze["phase_one_exact_delta"] == PHASE_ONE_EXACT_DELTA
    assert freeze["phase_one_exact_git_modes"] == PHASE_ONE_EXACT_GIT_MODES
    assert freeze[
    ...
    assert freeze["bundle_verifier_is_materialized_from_the_exact_source_commit_before_upload"]
    assert freeze[
    ...
    assert freeze["actual_host_prerequisite_preflight_requires_its_explicit_frozen_source_sha256"]
    assert freeze[
    ...
    labels = freeze["client_logged_boundary_labels_in_order"]
    prefixes = [f"{index:02d}-" for index in range(19)]
    assert len(labels) == len(set(labels)) == 19
    assert all(label.startswith(prefix) for label, prefix in zip(labels, prefixes, strict=True))
    assert freeze[
        "client_logged_boundary_labels_are_unique_no_overwrite_and_terminal_after_state_creation"
    ]
    assert freeze["mutable_worktree_client_tools_are_forbidden_at_every_real_host_boundary"]
    assert freeze[
        "runtime_receipt_publication_and_independent_review_precede_localization_dispatch"
    ]
...
def test_runbook_uses_complete_verifier_cli_and_clean_root_environment_handoffs() -> None:
    ...
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    host = contract["host_orchestration"]

    assert "ENGINEERING_REHEARSAL_POLICY.md" in runbook
    assert (
        "This pre-execution v6 revision keeps v5's corrected\nreceipt-review-seal order" in runbook
    )
    assert "scripts/run_logged_client_command.py" in runbook
    assert "python3 -I -S scripts/run_logged_client_command.py" not in runbook
    assert "p30_run_logged_client_command" in runbook
    assert (
        "export P30_CLIENT_PYTHON=/opt/homebrew/Cellar/python@3.14/3.14.3_1/"
        "Frameworks/Python.framework/Versions/3.14/bin/python3.14"
    ) in runbook
    assert (
        "export P30_EXPECTED_CLIENT_PYTHON_SHA256="
        "f13707ff725eb3675d57e0f04527a4dada0511cebfb7cbd80f5cab72733a2a9d"
    ) in runbook
    assert (
        "export P30_EXPECTED_CLIENT_LOGGER_SOURCE_SHA256="
        + hashlib.sha256((ROOT / "scripts/run_logged_client_command.py").read_bytes()).hexdigest()
    ) in runbook
    assert (
        "export P30_EXPECTED_LOCAL_BUNDLE_VERIFIER_SOURCE_SHA256="
        + hashlib.sha256((ROOT / "scripts/verify_p30_local_bundle.py").read_bytes()).hexdigest()
    ) in runbook
    assert (
        "export P30_EXPECTED_AUTHENTICATED_TRANSFER_SOURCE_SHA256="
        + hashlib.sha256(
            (ROOT / "scripts/run_p30_authenticated_client_transfer.py").read_bytes()
        ).hexdigest()
    ) in runbook
    assert "assert sys.version_info[:3] == (3,14,3)" in runbook
    assert 'export P30_CLIENT_UID="$({ /usr/bin/id -u; })"' in runbook
    assert 'export P30_CLIENT_GID="$({ /usr/bin/id -g; })"' in runbook
    assert '/usr/bin/chgrp "$P30_CLIENT_GID" "$P30_CLIENT_SESSION_ROOT"' in runbook
    assert '})" = "$P30_CLIENT_UID:$P30_CLIENT_GID:700"' in runbook
    group_fix = runbook.index('/usr/bin/chgrp "$P30_CLIENT_GID" "$P30_CLIENT_SESSION_ROOT"')
    logger_materialization = runbook.index(
        "p30_materialize_frozen_client_source \\\n  scripts/run_logged_client_command.py"
    )
    assert group_fix < logger_materialization
    transport_start = runbook.index('export P30_LOCAL_TRANSPORT="$(/usr/bin/mktemp')
    transport_group_fix = runbook.index('/usr/bin/chgrp "$P30_CLIENT_GID" "$P30_LOCAL_TRANSPORT"')
    transport_authority_check = runbook.index(
        '})" = "$P30_CLIENT_UID:$P30_CLIENT_GID:700"', transport_start
    )
    transport_bundle = runbook.index('p30_local_git bundle create "$P30_LOCAL_SOURCE_BUNDLE"')
    assert transport_start < transport_group_fix < transport_authority_check < transport_bundle
    assert runbook.count('"$P30_CLIENT_PYTHON" -I -S "$P30_FROZEN_CLIENT_LOGGER"') == 1
    helper = runbook.split("p30_run_logged_client_command() {", 1)[1].split("\n}", 1)[0]
    assert '"${p30_client_forbidden_roots[@]}"' in helper
    assert 'forbidden_arguments+=(--forbidden-root "$forbidden_root")' in helper
    assert '"${forbidden_arguments[@]}"' in helper
    assert (
        '--expected-logger-source-sha256 \\\n      "$P30_EXPECTED_CLIENT_LOGGER_SOURCE_SHA256"'
    ) in helper
    assert 'export P30_RUNTIME_REVIEW_WORKTREE="$P30_LOCAL_REPOSITORY"' in runbook
    assert 'p30_client_forbidden_roots+=("$P30_LOCAL_TRANSPORT")' in runbook
    assert runbook.index('p30_client_forbidden_roots+=("$P30_LOCAL_TRANSPORT")') < (
        runbook.index("02-source-bundle-upload")
    )
    assert 'python3 -I -S "$P30_FROZEN_CLIENT_LOGGER"' not in runbook
    assert '/usr/bin/python3 -I -S "$P30_FROZEN_RECEIPT_REVIEWER"' not in runbook
    assert runbook.count('"$P30_CLIENT_PYTHON" -I -S "$P30_FROZEN_RECEIPT_REVIEWER"') == 2
    assert runbook.count('"$P30_CLIENT_PYTHON" -I -S "$P30_FROZEN_LOCAL_BUNDLE_VERIFIER"') == 2
    assert runbook.count('"$P30_CLIENT_PYTHON" -I -S "$P30_FROZEN_AUTHENTICATED_TRANSFER"') == 2
    assert runbook.count("--expected-transfer-source-sha256") == 2
    forbidden_roots = runbook.split("p30_client_forbidden_roots=(", 1)[1].split("\n)", 1)[0]
    assert '"$P30_CLIENT_TRANSFER_ROOT"' in forbidden_roots
    assert "tests/test_p30_authenticated_client_transfer.py" in runbook
    assert "uv run --locked python -I -S" not in runbook
    assert "outside this repository" in runbook
    assert "cat-file blob \\" in runbook
    assert '"$P30_SOURCE_FREEZE_COMMIT:$relative_path"' in runbook
    assert "os.O_WRONLY|os.O_CREAT|os.O_EXCL" in runbook
    assert "stat.S_IMODE(after.st_mode) != 0o400" in runbook
    assert '--expected-client-source-sha256 "$P30_EXPECTED_HOST_PREFLIGHT_SOURCE_SHA256"' in runbook
    assert '< "$P30_FROZEN_RECEIPT_REVIEWER"' not in runbook
    assert "< scripts/review_p30_control_bundle_receipt.py" not in runbook
    assert '"$P30_REMOTE_RECEIPT_REVIEWER"' not in runbook
    assert (
        runbook.count(
    ...
    assert runbook.count("/usr/bin/ssh -T -F /dev/null -p 22") == 1
    assert runbook.count('"${p30_strict_ssh[@]}"') >= 10
    assert "-o IdentitiesOnly=yes" in runbook
    assert "-o ProxyCommand=none" in runbook
    assert "-o ProxyJump=none" in runbook
    assert (
        "export P30_EXPECTED_HOST_KEY_FINGERPRINT="
    ...
    assert "flags = os.O_RDWR | os.O_CREAT | os.O_EXCL | os.O_CLOEXEC | os.O_NOFOLLOW" in runbook
    assert "stat.S_IMODE(created.st_mode) != 0o400" in runbook
    assert "created.st_nlink != 1" in runbook
    assert "stored != canonical" in runbook
    assert "ssh-keyscan key does not match the out-of-band fingerprint" in runbook
    assert (
        "The operator-confirmed authority is the host-side\nfingerprint obtained out of band"
        in runbook
    )
    ...
    assert 'test "${#p30_phase_one_paths[@]}" -eq 26' in runbook
    assert "export P30_HISTORICAL_V5_COMMIT=8286d45db45c27e219724cfb9ace2108a2823301" in runbook
    assert (
        "export P30_HISTORICAL_V5_TAG_OBJECT=4c0b848bee12efb27dc93276cd1866c8ddc2d0b3"
    ) in runbook
    assert 'p30_local_git add -- "${p30_phase_one_paths[@]}"' in runbook
    assert "p30_local_git diff --cached --no-renames --name-status" in runbook
    assert 'p30_local_git ls-files -s -- "${p30_phase_one_paths[@]}"' in runbook
    assert "p30_local_git diff --cached --check" in runbook
    assert 'test -z "$({ p30_local_git diff --no-ext-diff --name-only; })"' in runbook
    assert 'test -z "$({ p30_local_git ls-files --others --exclude-standard; })"' in runbook
    assert "p30_local_git commit --amend -m \\" in runbook
    assert "p30_local_git diff-tree --no-commit-id --name-status -r" in runbook
    assert 'p30_local_git ls-tree -r --full-tree "$P30_SOURCE_FREEZE_COMMIT" --' in runbook
    for delta_entry in PHASE_ONE_EXACT_DELTA:
        status, path = delta_entry.split("\t", 1)
        assert f"$'{status}\\t{path}'" in runbook
    for mode_entry in PHASE_ONE_EXACT_GIT_MODES:
        mode, path = mode_entry.split("\t", 1)
        assert f"$'{mode}\\t{path}'" in runbook
    assert runbook.count('--execution-root "$P30_EXECUTION_ROOT"') == 2
    for label in contract["two_phase_freeze"]["client_logged_boundary_labels_in_order"]:
        assert label in runbook
    assert '09-source-root-handoff 29 "${p30_source_root_environment[@]}"' in runbook
    assert '17-localization-root-handoff 37 "${p30_localization_root_environment[@]}"' in runbook
    assert runbook.count("/usr/bin/sudo -n /usr/bin/env -i") == 5
    assert runbook.count("p30_verify_root_environment_handoff() {") == 2
    assert runbook.count("<<'P30_ROOT_ENVIRONMENT_CHECK'") == 2
    assert runbook.count("P30_ROOT_ENVIRONMENT_CHECK\n}") == 2
    source_names = _runbook_array_names(runbook, "p30_source_root_environment")
    localization_names = _runbook_array_names(runbook, "p30_localization_root_environment")
    runtime_only_names = host["runtime_only_required_environment"]
    assert len(source_names) == len(set(source_names)) == 29
    assert source_names == host["source_required_environment"]
    assert len(localization_names) == len(set(localization_names)) == 37
    assert localization_names == host["required_environment"]
    assert set(localization_names) == set(source_names) | set(runtime_only_names)
    assert set(source_names).isdisjoint(runtime_only_names)
    assert "--preserve-env" in runbook
    assert "Do not rely on `sudo` environment inheritance or\n`--preserve-env`" in runbook

    handoff_marker = "<<'P30_ROOT_ENVIRONMENT_CHECK'\n"
    program_start = runbook.index(handoff_marker) + len(handoff_marker)
    program_end = runbook.index("\nP30_ROOT_ENVIRONMENT_CHECK", program_start)
...
def test_runtime_review_control_artifact_authority_is_explicit() -> None:
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    verifier = contract["bundle_verifier"]

    assert verifier["runtime_review_control_checkout_artifact_owner_uid"] == 1000
    assert verifier["runtime_review_control_checkout_artifact_owner_gid"] == 1000
    assert verifier["runtime_review_control_checkout_artifact_physical_mode"] == "0600"
    assert verifier["runtime_review_control_checkout_artifact_git_mode"] == "100644"
    assert verifier["runtime_review_control_checkout_artifact_nonsymlink"] is True
    assert verifier["runtime_review_control_checkout_artifact_single_link"] is True
    assert verifier["runtime_review_control_checkout_artifact_acl_free"] is True
...
def test_receipts_two_file_review_and_no_training_boundary_are_exact() -> None:
    ...
    assert closure["bundle_upload_source_identity_is_revalidated_after_emission"]
    ...
    assert orchestration["p27_sanitizer_native_mode_bridge"] == {
        "purpose": (
            "satisfy the unchanged P27 sanitizer's exact private-input mode after P30 "
            "has sealed the retained native artifact"
        ),
        "initial_owner_mode": "root:root 0444",
        "temporary_owner_mode": "root:root 0600",
        "final_owner_mode": "root:root 0444",
        "mode_change_only": True,
        "mode_change_only_excludes_the_expected_ctime_change": True,
        "nofollow_single_link_regular_file_required": True,
        "sha256_and_byte_count_bound_before_mode_change": True,
        "device_and_inode_from_the_initial_binding_are_required_by_the_mode_bridge": True,
        "device_inode_owner_link_count_size_mtime_stable_during_mode_change": True,
        "sha256_rechecked_after_mode_change": True,
        "reseal_attempted_after_every_return_from_the_sanitizer": True,
        "successful_sanitization_requires_resealed_sha256_and_byte_count_match": True,
        "p27_sanitizer_source_bytes_changed": False,
    }

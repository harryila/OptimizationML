...
SCRIPT = ROOT / "scripts/run_p29_permission_safe_bundle_localization_bridge.sh"
...
def _run_logger_status_fault(
    ...
    harness = f"""
set -uo pipefail
P29_HOST_EVIDENCE={shlex.quote(str(evidence))}
P29_DEFERRED_ROOT={shlex.quote(str(deferred))}
...
validate_deferred_journal() {{
  ...
  if [[ "$LOGGER_NAME" == run_root_journal_logged ]]; then
    ...
    for retained in "$EXPECTED_STDOUT" "$EXPECTED_STDERR" "$EXPECTED_STATUS"; do
      [[ "$(stat -f '%Lp' "$retained")" == 400 ]] || return 83
    done
  fi
  return 0
}}
...
def _run_token_creator(
    ...
    orchestrator = ledger / "run_p29_permission_safe_bundle_localization_bridge.sh"
...
def test_attempt_image_gpu_and_authority_are_fixed_and_env_checked() -> None:
    source = _source()
    assert "readonly P29_NAMESPACE_ROOT=/secure/p29" in source
    assert "readonly P29_ATTEMPT_ROOT=/secure/p29/attempt-20260906-02" in source
    assert "readonly P29_CONTAINER=p29-localization-20260906-02" in source
    assert "readonly P29_TRANSPORT_ROOT=/secure/p29/transport-20260906-02" in source
    assert (
        'readonly P29_BOOTSTRAP_VERIFIER="$P29_TRANSPORT_ROOT/verify_p29_control_bundle.py"'
        in source
    )
    assert 'readonly P29_CONTROL_REPO="$P29_TRANSPORT_ROOT/control-source"' in source
    ...
    assert "require_var P29_ATTEMPT_ID" in required
    assert '"$P29_ATTEMPT_ID" == "$P29_REQUIRED_ATTEMPT_ID"' in required
    assert "require_var P29_GPU_UUID" in required
    assert '"$P29_GPU_UUID" == "$P29_REQUIRED_GPU_UUID"' in required
    assert '"$P29_AUTHORITY_REPO" == "$P29_TRANSPORT_ROOT/authority-185e444"' in required
...
def test_source_bundle_receipt_and_all_reconstructions_precede_attempt_write() -> None:
    ...
    admission_log = prepare.index("run_root_journal_logged p29-prepare-pre-attempt-admission")
    attempt_write = prepare.index("run_logged_deferred_evidence p29-prepare-attempt-layout")
    container_write = prepare.index("refresh_bound_file p29-container-launch")
    ...
    assert "run_p29_reconstruction_unlogged" not in admission
    ...
    assert '--bootstrap-verifier "$P29_BOOTSTRAP_VERIFIER"' in replay
    assert '--expected-verifier-sha256 "$P29_EXPECTED_BUNDLE_VERIFIER_SHA256"' in replay
    assert '"$P29_SOURCE_BUNDLE"' in replay
    assert '"$P29_SOURCE_CLOSURE"' in replay
    assert '"$P29_SOURCE_RECEIPT"' in replay
    assert "--verify-existing-receipt" in replay
    assert "--receipt-output" not in replay
    assert '"$P29_EXPECTED_SOURCE_RECEIPT_SHA256"' in replay
...
def test_runtime_review_receipt_replay_precedes_localization_marker() -> None:
    ...
    entry_preflight = run.index("run_logged p29-localization-entry-preflight")
    ingestion = run.index("run_logged p29-p26-failure-ingestion")
    localization = run.index("run_logged p29-localization run_localizer_once")
    ...
    assert '--bootstrap-verifier "$P29_BOOTSTRAP_VERIFIER"' in replay
    assert '--expected-verifier-sha256 "$P29_EXPECTED_BUNDLE_VERIFIER_SHA256"' in replay
    assert '"$P29_RUNTIME_REVIEW_BUNDLE"' in replay
    assert '"$P29_RUNTIME_REVIEW_CLOSURE"' in replay
    assert '"$P29_RUNTIME_REVIEW_RECEIPT"' in replay
...
P29_DEFERRED_ROOT={shlex.quote(str(deferred))}
P29_HOST_EVIDENCE={shlex.quote(str(evidence))}
P29_RUNTIME_REVIEW_RECEIPT={shlex.quote(str(tmp_path / "receipt.json"))}
OBSERVED_STATUS={shlex.quote(str(observed_status))}
unset P29_EXPECTED_RUNTIME_REVIEW_RECEIPT_SHA256
...
set +e
run_logged_deferred_evidence \
  p29-pre-marker-runtime-receipt-replay failing_runtime_verifier
logger_status=$?
set -e
...
"""
    ...
    prefix = deferred / ".deferred-p29-pre-marker-runtime-receipt-replay"
...
def test_prepare_uses_exact_p23_ten_mount_contract() -> None:
    ...
    assert "p29_docker run" in _function(
    ...
    assert "run_p29_permission_safe_bundle_localization_bridge.py" not in prepare
...
def test_persistent_authority_checkout_rejects_git_object_indirections() -> None:
    ...
    assert '"$P29_AUTHORITY_REPO/.git/objects/info/alternates"' in authority
    assert '"$P29_AUTHORITY_REPO/.git/info/grafts"' in authority
    ...
    assert "P29 authority checkout has forbidden replace refs" in authority
...
def test_prepare_stops_at_freeze_and_has_no_cleanup_or_retry_commands() -> None:
    ...
    prepare = _function(source, "prepare_runtime", "stage_source")
    assert "run_logged p29-freeze-runtime freeze_runtime_from_bound_container" in prepare
    assert "run_logged p29-freeze-runtime freeze_runtime_from_bound_container ||" not in prepare
    assert prepare.index("set +e\n  run_logged p29-freeze-runtime") < prepare.index(
        "freeze_status=$?\n  set -e"
    )
    freeze = _function(
        source, "freeze_runtime_from_bound_container", "capture_running_container_inspection"
    )
    assert "p29_cuda_runtime_lock.json" in freeze
    assert "p29_host_attestation.json" in freeze
...
"""
    ...
    assert (evidence / "p29-freeze-runtime.exit-status.txt").read_bytes() == b"1\n"
...
def test_runtime_review_requires_direct_child_and_exact_two_file_delta() -> None:
    ...
    assert '"$P29_SOURCE_FREEZE_COMMIT^{tree}"' in source_history
    assert '"$P29_SOURCE_FREEZE_TREE"' in source_history
    assert "$P29_SOURCE_FREEZE_COMMIT $P29_TERMINAL_P28_COMMIT" in source_history
    assert "terminal P28 commit/tree binding differs" in source_history
    review = _function(source, "validate_runtime_review_history", "run_contract_reconstruction")
    assert "$P29_RUNTIME_REVIEW_COMMIT $P29_SOURCE_FREEZE_COMMIT" in review
    assert review.count("A\\texperiments/training/p29_cuda_runtime_lock.json") == 1
    assert review.count("A\\texperiments/training/p29_host_attestation.json") == 1
...
def test_permission_safe_namespace_and_attempt_layouts_are_revalidated() -> None:
    ...
    assert 'pathlib.PurePosixPath("/secure/p29")' in namespace
    assert 'namespace / "transport-20260906-02"' in namespace
    ...
    assert 'namespace / "attempt-20260906-02"' in attempt
    assert "P29 attempt component authority differs" in attempt
    prepare = _function(source, "prepare_runtime", "stage_source")
...
def test_executing_orchestrator_is_bound_to_reviewed_control_bytes() -> None:
    ...
    assert '"$executing_orchestrator" == "$P29_SEALED_ORCHESTRATOR"' in validation
    assert "executing P29 host orchestrator bytes differ" in validation
    ...
    assert "executing P29 host orchestrator bytes differ before transport replay" in (
        transport_validation
    )
    assert '"$(sha256_file "$P29_BOOTSTRAP_VERIFIER")"' in transport_validation
    assert '"$(/usr/bin/stat -c \'%a\' "$P29_BOOTSTRAP_VERIFIER")" == "600"' in (
        transport_validation
    )
...
def test_run_stages_exactly_four_hash_bound_sources_in_existing_evidence_mount() -> None:
    ...
    assert (
        "/private/tmp/optimizationml-p22-data/.p29-tools/"
    ...
    assert (
        "/private/tmp/optimizationml-p22-data/.p29-tools/p23_deterministic_cuda_shadow_trace.py"
    ) in run
    assert (
        "/private/tmp/optimizationml-p22-data/.p29-tools/"
...
def test_p26_ingestion_is_host_root_one_read_bridge_before_cuda_localizer() -> None:
    source = _source()
    run = _function(source, "run_localization", "usage")
    ingestion = run.index("run_logged p29-p26-failure-ingestion run_p26_ingestion_once")
    localization = run.index("run_logged p29-localization run_localizer_once")
    sanitizer = run.index("run_logged p29-localization-sanitizer sanitize_localization_artifact")
    assert ingestion < localization < sanitizer
    assert (
        "--source /private/tmp/optimizationml-p22-data/.p29-tools/"
        "p26-trace-off-a-failure.authenticated.json"
    ) in run
    assert "--output-root /workspace/evidence/p23" in run
    assert 'sha256_file "$P29_P26_NATIVE"' not in run
    assert 'root_sha256_file "$P29_P26_NATIVE"' not in run
...
def test_localizer_is_one_shot_and_preserves_nonzero_status_before_sanitizing() -> None:
    source = _source()
    run = _function(source, "run_localization", "usage")
    assert run.count("run_logged p29-localization run_localizer_once") == 1
    assert "run-localization.invoked" in source
    assert "no rerun is allowed" in run
    command = run[run.index("run_logged p29-localization run_localizer_once") :]
    assert command.count("--expected-p25-source-sha256") == 1
    assert command.count("--expected-p25-contract-sha256") == 1
    sanitizer = run.index("run_logged p29-localization-sanitizer sanitize_localization_artifact")
    assert run.index("run_logged p29-localization run_localizer_once") < sanitizer
    assert "--output-root /workspace/evidence/p23" in run
    assert (
        "--runner-source /private/tmp/optimizationml-p22-data/.p29-tools/"
        "run_p27_cuda_deleted_mapping_localization.py"
    ) in run
...
def test_live_revalidation_covers_identity_mounts_gpu_and_runtime_bytes() -> None:
    ...
    run = _function(source, "run_localization", "usage")
    assert "validate_live_runtime p29-pre-ingestion" in run
    assert "validate_live_runtime p29-pre-localization" in run
    ...
    assert '"$native_lock" "$P29_EXPECTED_RUNTIME_LOCK_SHA256" - 0444 0 0' in artifacts
    assert '"$native_attestation" "$P29_EXPECTED_HOST_ATTESTATION_SHA256" - 0444 0 0' in artifacts
...
set -uo pipefail
TRACE={shlex.quote(str(trace))}
P29_PREPARE_INVOCATION={shlex.quote(str(token))}

{burn}

create_localization_token() {{
  [[ "$1" == prepare ]] || return 90
  if [[ -f "$P29_PREPARE_INVOCATION" &&
        "$(<"$P29_PREPARE_INVOCATION")" == prepare-runtime ]]; then
...
def test_token_creation_authenticates_journal_and_sealed_orchestrator_before_o_excl() -> None:
    ...
    assert '"run_p29_permission_safe_bundle_localization_bridge.sh"' in before_token
    ...
    assert "$P29_EXPECTED_ORCHESTRATOR_SHA256" in function_source.split("<<'PY'", 1)[0]
...
def test_complete_prepare_admission_is_root_journal_logged_before_attempt_creation() -> None:
    source = _source()
    prepare = _function(source, "prepare_runtime", "validate_live_runtime")
    admission_call = prepare.index("run_root_journal_logged p29-prepare-pre-attempt-admission")
    attempt_call = prepare.index("run_logged_deferred_evidence p29-prepare-attempt-layout")
...
def test_deferred_journal_inventory_includes_three_prepare_admission_files() -> None:
    ...
    for suffix in ("stdout.log", "stderr.log", "exit-status.txt"):
        assert f'".deferred-p29-prepare-pre-attempt-admission.{suffix}"' in journal
...
def test_logger_status_failure_after_producer_is_terminal_and_seals_every_output(
    ...
    if logger_name in {
        ...
        label = (
            "p29-prepare-pre-attempt-admission"
            if logger_name == "run_root_journal_logged"
            else "p29-prepare-attempt-layout"
        )
...
def test_root_admission_logger_retains_producer_status_and_validates_transcripts(
    ...
    prefix = tmp_path / "deferred/.deferred-p29-prepare-pre-attempt-admission"
    for suffix in ("stdout.log", "stderr.log", "exit-status.txt"):
        assert stat.S_IMODE(Path(f"{prefix}.{suffix}").stat().st_mode) == 0o400
...
set -uo pipefail
TRACE={shlex.quote(str(trace))}
P29_LOCALIZATION_INVOCATION={shlex.quote(str(invocation))}
P29_LOCALIZATION_EXIT_STATUS={shlex.quote(str(terminal_status))}

{burn}

create_localization_token() {{
  [[ "$1" == invocation ]] || return 90
  printf 'run-localization\n' >"$P29_LOCALIZATION_INVOCATION"
...
write_localization_exit_status() {{
  ...
  printf '%s\n' "$1" >"$P29_LOCALIZATION_EXIT_STATUS"
  return 0
}}
...
create_localization_token() {{
  [[ "$1" == invocation ]] || return 90
  if [[ ! -e "$P29_LOCALIZATION_INVOCATION" ]]; then
    printf 'run-localization\n' >"$P29_LOCALIZATION_INVOCATION"
...
"""
    ...
    replay = subprocess.run(
        ["bash", "-c", harness, "p29-test", "replay"],
        cwd=tmp_path,
        text=True,
        capture_output=True,
        check=False,

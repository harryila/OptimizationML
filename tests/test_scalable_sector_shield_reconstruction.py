...
def _run(root: Path, *arguments: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, str(root / "scripts/reconstruct_scalable_sector_shield.py"), *arguments],
        cwd=root,
        check=check,
        capture_output=True,
        text=True,
...
def _generate(root: Path, output: Path) -> None:
    subprocess.run(
        [
            sys.executable,
            str(root / "scripts/certify_scalable_sector_shield.py"),
            "--output",
            str(output),
        ],
        cwd=root,
...
def test_p20_reconstruction_is_standard_library_only() -> None:
    root = Path(__file__).resolve().parents[1]
    script = root / "scripts/reconstruct_scalable_sector_shield.py"
...
def test_p20_reconstruction_closes_internal_checks() -> None:
    root = Path(__file__).resolve().parents[1]
    payload = json.loads(_run(root).stdout)
    assert payload["schema_version"] == "passive-muon-p20-independent-reconstruction-v1"
...
def test_p20_reconstruction_matches_fresh_generator(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    fresh = tmp_path / "fresh-p20.json"
...
def test_p20_reconstruction_matches_committed_canonical_when_present() -> None:
    root = Path(__file__).resolve().parents[1]
    canonical = root / "results/summaries/scalable_sector_shield_certificate.json"
    if not canonical.is_file():
        pytest.skip("committed P20 canonical certificate has not landed yet")
...
def test_p20_reconstruction_rejects_tampered_margin(tmp_path: Path) -> None:
    ...
    payload["reconstruction_fields"]["shapes"][-1]["margin"]["inward_margin"] = "0"
    tampered = tmp_path / "tampered-p20.json"

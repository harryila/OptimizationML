...
@pytest.mark.parametrize(
    ...
    [
        (41, (0, 41)),
        (64769, (253, 1)),
        (0x120006783459A, (0x12345, 0x6789A)),
    ],
)
def test_linux_device_components_are_review_host_independent(
    encoded: int, expected: tuple[int, int]
) -> None:
    assert reviewer._linux_device_components(encoded) == expected


@pytest.mark.parametrize("encoded", [-1, 1 << 64, True, "64769"])
def test_linux_device_components_reject_malformed_values(encoded: object) -> None:
    with pytest.raises(reviewer.ReceiptReviewError, match="device encoding"):
        reviewer._linux_device_components(encoded)  # type: ignore[arg-type]


def test_client_download_postcheck_uses_pinned_macos_paths() -> None:
    runbook = RUNBOOK.read_text(encoding="utf-8")
    helper = runbook.split("p30_o_excl_download() {", 1)[1].split("\n}", 1)[0]

    assert '/bin/test "$({ /bin/cat ' in helper
    assert "/usr/bin/test" not in helper
    assert "/usr/bin/cat" not in helper
...
def test_linux_receipt_device_is_not_decoded_with_review_host_os(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    receipt = _receipt("source")

    def replace_device(value: object) -> None:
        if isinstance(value, dict):
            for key, child in value.items():
                if key in {"device", "bundle_device"} and child == DEVICE:
                    value[key] = 64769
                elif key == "containing_mount_device":
                    value[key] = "253:1"
                else:
                    replace_device(child)
        elif isinstance(value, list):
            for child in value:
                replace_device(child)

    replace_device(receipt)
    monkeypatch.setattr(
        reviewer.os,
        "major",
        lambda _device: (_ for _ in ()).throw(AssertionError("host decoder used")),
    )
    monkeypatch.setattr(
        reviewer.os,
        "minor",
    ...
    assert reviewer.review_receipt(raw, _expectations("source", raw))["passes"] is True

...
RECONSTRUCTOR = ROOT / "scripts/reconstruct_p29_permission_safe_bundle_localization_bridge.py"
WORKFLOW = ROOT / ".github/workflows/p29-permission-safe-bundle-localization-bridge.yml"
RUNBOOK = ROOT / "experiments/training/P29_PERMISSION_SAFE_BUNDLE_LOCALIZATION_BRIDGE_RUNBOOK.md"
PathPart: TypeAlias = str | int


def _module():
    specification = importlib.util.spec_from_file_location(
        "p29_contract_reconstruction", RECONSTRUCTOR
    )
...
def test_p29_permission_safe_bridge_reconstructs() -> None:
...
def test_runbook_uses_complete_verifier_cli_and_clean_root_environment_handoffs() -> None:
    ...
    assert runbook.count('--execution-root "$P29_EXECUTION_ROOT"') == 2
    assert 'p29_verify_root_environment_handoff 25 "${p29_source_root_environment[@]}"' in runbook
    assert (
        'p29_verify_root_environment_handoff 33 "${p29_localization_root_environment[@]}"'
        in runbook
    )
    assert runbook.count("sudo /usr/bin/env -i \\\n") == 4
    assert runbook.count("p29_verify_root_environment_handoff() {") == 2
...
def test_terminal_p28_parent_and_unchanged_sources_are_bound() -> None:
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    parent = contract["terminal_parent"]

    assert parent["outcome_commit"] == "7274367f2b05fb3c9ed9f876a59be647703bbdc2"
    assert parent["outcome_tree"] == "8d971e176e664ea47d8f769ae54f8cf349ceb3d5"
    assert parent["diagnostic_tag_object"] == "f3c81a2f14f853f369015a4f81b6695b52eec74e"
    assert parent["source_verifier_invocations_run"] == 1
    assert parent["source_receipt_created"] is False
    assert parent["control_checkout_created"] is False
    ...
    for group_name in ("unchanged_p28_authorities", "unchanged_p27_authorities"):
...
def test_bundle_inventory_carries_all_seven_exact_refs_and_annotated_tags() -> None:
    ...
    assert refs[2]["object_type"] == refs[4]["object_type"] == "tag"
    assert refs[5]["object_type"] == refs[6]["object_type"] == "tag"
    assert refs[2]["object"] == "f3c81a2f14f853f369015a4f81b6695b52eec74e"
    assert refs[2]["peeled_commit"] == "7274367f2b05fb3c9ed9f876a59be647703bbdc2"
    assert refs[5]["object"] == "bacad707d3779bfa10957e18cb4c69b1a7f0cbce"
    assert refs[6]["object"] == "f35a7dca8f6bc39e9748e79b6712fab4a203396b"
...
def test_permission_layout_uses_one_narrow_process_owned_capability_root() -> None:
    ...
    assert layout["localization_ledger_root"] == "/var/lib/optimizationml-p29-20260906-02"
    assert (
    ...
    assert layout["deferred_journal_root"] == ("/var/lib/optimizationml-p29-20260906-02/deferred")
    ...
    assert layout["source_phase_exact_secure_root_child_inventory"] == ["transport-20260906-02"]
    ...
    assert attempt["attempt_root"] == "/secure/p29/attempt-20260906-02"
    assert attempt["evidence_root"] == "/secure/p29/attempt-20260906-02/evidence"
...
def test_receipts_two_file_review_and_no_training_boundary_are_exact() -> None:
    ...
    assert closure["exact_advertised_ref_count"] == 7
    assert closure["bundle_prerequisite_count"] == 0
    assert closure["receipt_schema"] == "passive-muon-p29-control-bundle-receipt-v1"
    ...
    assert verifier["source_phase_exact_namespace_child_inventory"] == ["transport-20260906-02"]
    ...
    assert len(orchestration["required_environment"]) == 33
    assert len(set(orchestration["required_environment"])) == 33
    assert orchestration["required_environment"][-1] == "P29_EXPECTED_CONTAINER_ID"
    ...
    assert orchestration["root_environment_handoff_read_only_preflight_creates_no_p29_state"]
    assert orchestration[
    ...
    assert (
        orchestration["premature_localization_after_successful_prepare_token_exists_consumes_p29"]
        is True
    )
    assert (
        orchestration[
    ...
    assert (
        orchestration["independent_p29_git_shim_executes_absolute_git_with_frozen_trust_controls"]
        is True
    )
...
def test_immutable_execution_snapshot_and_ten_mounts_are_exact() -> None:
    ...
    assert execution["root"] == "/secure/p29/execution-20260906-02"
    ...
    assert execution["manifest_schema"] == "passive-muon-p29-immutable-execution-snapshot-v1"
    ...
    assert mounts[5]["source"] == "/secure/p29/attempt-20260906-02/evidence"
...
def test_workflow_enters_prior_authorities_only_through_hardened_p29_reconstructor() -> None:
    workflow = WORKFLOW.read_text(encoding="utf-8")

    assert "uv run --locked python -I -S" in workflow
    assert "scripts/reconstruct_p29_permission_safe_bundle_localization_bridge.py" in workflow

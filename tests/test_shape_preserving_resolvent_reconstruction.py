...
def _run(root: Path, *arguments: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            str(root / "scripts/reconstruct_shape_preserving_resolvent.py"),
            *arguments,
        ],
        cwd=root,
        check=check,
...
def test_p17_reconstruction_is_standard_library_only() -> None:
    root = Path(__file__).resolve().parents[1]
    script = root / "scripts/reconstruct_shape_preserving_resolvent.py"
...
def test_p17_reconstruction_closes_internal_exact_checks() -> None:
    root = Path(__file__).resolve().parents[1]
    payload = json.loads(_run(root).stdout)

    assert payload["schema_version"] == "passive-muon-p17-independent-reconstruction-v1"
...
def test_p17_reconstruction_matches_fresh_generator(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    fresh = tmp_path / "fresh-p17.json"
...
def test_p17_reconstruction_matches_committed_canonical_when_present() -> None:
    root = Path(__file__).resolve().parents[1]
    canonical = root / "results/summaries/shape_preserving_resolvent_certificate.json"
    if not canonical.is_file():
        pytest.skip("committed P17 canonical certificate has not landed yet")
    payload = json.loads(_run(root, "--require-canonical").stdout)
    assert payload["canonical_comparison"]["status"] == "matched"
    assert payload["all_exact_checks_passed"]


def test_p17_reconstruction_rejects_tampered_gain_bound(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    fresh = tmp_path / "fresh-p17.json"
    _generate(root, fresh)
    payload = json.loads(fresh.read_text(encoding="utf-8"))
    payload["reconstruction_fields"]["raw_shape_gain_upper"] = "0"
    tampered = tmp_path / "tampered-p17.json"

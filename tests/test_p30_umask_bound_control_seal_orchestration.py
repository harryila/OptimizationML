...
P27_SANITIZER_TEST = ROOT / "tests/test_p27_cuda_deleted_mapping_localization_sanitizer.py"
...
def _mode_bridge_python(*, inject_after_fchmod_failure: bool = False) -> str:
    function_source = _function(
        _source(),
        "set_native_private_mode_for_p27_sanitizer",
    ...
    python_source = python_source.replace("(0, 0, 0o555)", f"({uid}, {gid}, 0o555)").replace(
        "(0, 0, 0o444)", f"({uid}, {gid}, 0o444)"
    )
    python_source = python_source.replace("os.listxattr(target)", "[]")
    # The production bridge runs as UID 0 and deliberately opens a sealed
    # 0444 file O_RDWR.  The portable owner-only fixture cannot open 0444 for
    # writing, so use O_RDONLY here; fchmod and all identity/hash checks remain
    # the production code path.  The disposable root rehearsal covers UID 0.
    python_source = python_source.replace("os.O_RDWR", "os.O_RDONLY", 1)
    if inject_after_fchmod_failure:
        needle = "    os.fchmod(descriptor, 0o600)\n    os.fsync(descriptor)"
        assert needle in python_source
        python_source = python_source.replace(
            needle,
            needle + "\n    raise OSError('injected post-fchmod bridge failure')",
            1,
        )
...
def test_mode_bridge_composes_with_unchanged_p27_sanitizer_and_reseal(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    specification = importlib.util.spec_from_file_location(
        "p30_p27_sanitizer_fixture", P27_SANITIZER_TEST
    )
    assert specification is not None and specification.loader is not None
    fixture = importlib.util.module_from_spec(specification)
    specification.loader.exec_module(fixture)
    sanitizer, runner = fixture._modules()

    evidence = tmp_path / "evidence"
    evidence.mkdir(mode=0o755)
    native = evidence / "native.json"
    digest, byte_count = fixture._write_native(native, fixture._success_payload(runner))
    native.chmod(0o444)
    evidence.chmod(0o555)
    initial_inode = native.stat().st_ino
    ...
    bridge = subprocess.run(
        [
            ...
            digest,
            str(byte_count),
            str(native.stat().st_dev),
            str(native.stat().st_ino),
        ],
        input=_mode_bridge_python(),
        text=True,
        capture_output=True,
        check=False,
    )
    ...
    sanitizer.sanitize(
        ...
        path_roots=fixture._roots(),
        output_root=tmp_path,
        runner_source_path=fixture.RUNNER_SOURCE,
        repository_root=ROOT,
    )
    native.chmod(0o444)
    assert native.stat().st_ino == initial_inode
    assert hashlib.sha256(native.read_bytes()).hexdigest() == digest
    assert stat.S_IMODE(native.stat().st_mode) == 0o444
    assert json.loads(output.read_bytes())["native_artifact"]["mode_octal"] == "0600"


def test_outer_inventory_seal_recovers_an_injected_post_fchmod_bridge_failure(
    tmp_path: Path,
) -> None:
    evidence = tmp_path / "evidence"
    evidence.mkdir(mode=0o755)
    ...
    bridge = subprocess.run(
        [
            ...
            str(evidence),
            digest,
            str(native.stat().st_size),
            str(native.stat().st_dev),
            str(native.stat().st_ino),
        ],
        input=_mode_bridge_python(inject_after_fchmod_failure=True),
        text=True,
        capture_output=True,
        check=False,
    )
    assert bridge.returncode != 0
    assert "injected post-fchmod bridge failure" in bridge.stderr
    assert stat.S_IMODE(native.stat().st_mode) == 0o600

    seal_source = _heredoc_python(_function(_source(), "seal_evidence_inventory", "run_logged"))
    uid = os.getuid()
    gid = os.getgid()
    seal_source = seal_source.replace("(0, 0, 0o555)", f"({uid}, {gid}, 0o555)").replace(
        "(0, 0, 0o444)", f"({uid}, {gid}, 0o444)"
    )
    seal_source = seal_source.replace("(0, 0)", f"({uid}, {gid})")
    seal_source = seal_source.replace("os.listxattr(file_descriptor)", "[]")
...
def test_sealed_native_mode_bridge_is_narrow_hash_bound_and_always_resealed() -> None:
    source = _source()
    sanitizer = _function(
        source, "sanitize_localization_artifact", "validate_localization_entry_state"
    )
    bridge = _function(
        source,
        "set_native_private_mode_for_p27_sanitizer",
    ...
    assert 'validate_root_evidence_file "$native" - - 0444 0 0' in sanitizer
    assert '"$native" "$native_sha256" "$native_byte_count"' in sanitizer
    assert '"$native_device" "$native_inode"' in sanitizer
    assert "--native-mode" not in sanitizer
    assert sanitizer.index('validate_root_evidence_file "$native" - - 0444 0 0') < (
        sanitizer.index("set_native_private_mode_for_p27_sanitizer")
    )
    assert sanitizer.index("set_native_private_mode_for_p27_sanitizer") < sanitizer.index(
        "p30_docker exec"
    )
    assert sanitizer.index("p30_docker exec") < sanitizer.index('seal_retained_file "$native"')
    assert "sanitizer_status=$?" in sanitizer
    assert "reseal_status=$?" in sanitizer
    assert '[[ "$rebound" == "$binding" ]]' in sanitizer
    assert "O_RDWR | os.O_CLOEXEC | os.O_NOFOLLOW" in bridge
    assert "(0, 0, 0o444)" in bridge
    assert "(before.st_dev, before.st_ino) != (expected_device, expected_inode)" in bridge
    assert "os.fchmod(descriptor, 0o600)" in bridge
    assert "digest_fd(descriptor) != expected_sha256" in bridge
    assert "stable(after) != stable(before)" in bridge
...
def test_live_revalidation_covers_identity_mounts_gpu_and_runtime_bytes() -> None:
    ...
    assert "stable_bytes(lock_path, 1000, 1000, 0o600" in live
    assert "attestation_path, 1000, 1000, 0o600" in live
    assert "stable_bytes(lock_path, 1000, 1000, 0o644" not in live
    assert "attestation_path, 1000, 1000, 0o644" not in live
    run = _function(source, "run_localization", "usage")

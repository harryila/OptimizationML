...
export P30_EXPECTED_RECEIPT_REVIEWER_SOURCE_SHA256=b0f68aa2a78ea98e4b5995f722bb386db592e2be73dfae31cec25358a91f203c
...
p30_o_excl_download() {
  ...
  /bin/test "$({ /bin/cat "$P30_CLIENT_LOG_ROOT/$label.exit-status.txt"; })" = 0
}
```

The download postcheck uses the portable absolute locations shared by the
pinned macOS client, `/bin/test` and `/bin/cat`.  Receipt mount-device review
decodes the retained Linux `dev_t` layout explicitly; it never applies the
review host's platform-specific `os.major` or `os.minor` implementation to a
Linux device number.

...
The immutable tag `p30-umask-bound-control-seal-source-freeze-v4` records the
original source freeze. The pre-execution revision after that tag corrects one
machine-readable ordering error: the source receipt is created, then reviewed,
and only then may the reviewed source be sealed. It does not alter P30's four
diagnostic stages, mapping classification, terminal routing, or no-training
boundary. The historical tag and every P23--P29 terminal record remain
unchanged.
...
verifier and all six reconstructions, reviews and replays the receipt, and
checks the root-owned mode-`0555` sealed bytes. It may mirror P30's absolute
path strings on the disposable runner, but it creates no attempt root and
cannot consume the real CUDA host's frozen identity. It does not initialize
...
operator and mode `0700`. Run each earliest client-side provisioning, source
verifier, receipt-review, and reviewed-seal command through:

```bash
python3 -I -S scripts/run_logged_client_command.py \
  --log-root "$P30_CLIENT_LOG_ROOT" \
  --label <unique-lowercase-label> -- <command> <arguments...>
```
...
`8a263722c7f8ce1d99b8796a3ccc0425dd1cefcf`. Record the resulting commit and
tree as `P30_SOURCE_FREEZE_COMMIT` and `P30_SOURCE_FREEZE_TREE`. Those values
...
```bash
export P30_LOCAL_TRANSPORT="$(mktemp -d /private/tmp/p30-transfer.XXXXXX)"
export P30_LOCAL_SOURCE_BUNDLE="$P30_LOCAL_TRANSPORT/p30_source.bundle"
/usr/bin/git bundle create "$P30_LOCAL_SOURCE_BUNDLE" \
...
/usr/bin/git bundle list-heads "$P30_LOCAL_SOURCE_BUNDLE"
/usr/bin/git bundle verify "$P30_LOCAL_SOURCE_BUNDLE"
shasum -a 256 "$P30_LOCAL_SOURCE_BUNDLE"
wc -c "$P30_LOCAL_SOURCE_BUNDLE"
```

`list-heads` must have exactly nine rows and the five tag rows must name tag
...
P30 namespace. Run these once on the CUDA host; if any command fails or any new
...
try:
    ...
    os.close(root)
PY
sudo /usr/bin/test ! -e /secure/p30
sudo /usr/bin/test ! -L /secure/p30
sudo /usr/bin/mkdir --mode=0755 /secure/p30
sudo /usr/bin/chown root:root /secure/p30
sudo /usr/bin/test ! -e /secure/p30/transport-20260906-03
sudo /usr/bin/test ! -L /secure/p30/transport-20260906-03
sudo /usr/bin/mkdir --mode=0700 /secure/p30/transport-20260906-03
sudo /usr/bin/chown ubuntu:ubuntu /secure/p30/transport-20260906-03
sudo /usr/bin/test ! -e /var/lib/optimizationml-p30-20260906-03
sudo /usr/bin/test ! -L /var/lib/optimizationml-p30-20260906-03
sudo /usr/bin/mkdir --mode=0700 /var/lib/optimizationml-p30-20260906-03
sudo /usr/bin/chown root:root /var/lib/optimizationml-p30-20260906-03
sudo /usr/bin/mkdir --mode=0700 /var/lib/optimizationml-p30-20260906-03/deferred
sudo /usr/bin/chown root:root /var/lib/optimizationml-p30-20260906-03/deferred
```
...
p30_o_excl_upload() {
  ...
  test -f "$source_path"
  ssh -o BatchMode=yes -o StrictHostKeyChecking=yes "$P30_SSH_TARGET" \
...
p30_o_excl_upload "$P30_LOCAL_SOURCE_BUNDLE" "$P30_SOURCE_BUNDLE" 0600
p30_o_excl_upload \
  scripts/verify_p30_control_bundle.py "$P30_BOOTSTRAP_VERIFIER" 0600
```
...
p30_verify_root_environment_handoff 29 "${p30_source_root_environment[@]}"
```
...
`P30_SOURCE_FREEZE_COMMIT`, with exactly:
...
`P30_EXPECTED_CONTAINER_ID`. The latter is the exact 64-lowercase-hex value
from the sealed `p30-container-id.txt`; it must independently match the
...
/usr/bin/git bundle create "$P30_LOCAL_RUNTIME_BUNDLE" \
  refs/heads/p30-umask-bound-control-seal \
  refs/heads/p29-permission-safe-bundle-localization-bridge \
  refs/tags/p29-orchestrator-source-mode-diagnostic \
  refs/heads/p28-bundle-complete-localization-bridge \
  ...
  refs/tags/p26-permission-safe-acquisition-checkpoint \
  refs/tags/p26-attempt02-acquisition-source
/usr/bin/git bundle list-heads "$P30_LOCAL_RUNTIME_BUNDLE"
/usr/bin/git bundle verify "$P30_LOCAL_RUNTIME_BUNDLE"
shasum -a 256 "$P30_LOCAL_RUNTIME_BUNDLE"
wc -c "$P30_LOCAL_RUNTIME_BUNDLE"
p30_o_excl_upload \
...
```bash
set -euo pipefail
p30_runtime_receipt_creation_logged() {
  /usr/bin/python3 -I -S - \
...
p30_runtime_receipt_creation_logged \
  ...
  --expected-verifier-sha256 "$P30_EXPECTED_BUNDLE_VERIFIER_SHA256" \
  --expected-orchestrator-sha256 "$P30_EXPECTED_ORCHESTRATOR_SHA256" \
  --receipt-output "$P30_RUNTIME_REVIEW_RECEIPT"
```

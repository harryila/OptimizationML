...
def _linux_device_components(device: int) -> tuple[int, int]:
    """Decode a Linux ``dev_t`` independently of the review host OS."""

    if type(device) is not int or not 0 <= device < (1 << 64):
        raise ReceiptReviewError("Linux device encoding is malformed")
...
def _review_permission_safe_namespace(
    ...
    linux_major, linux_minor = _linux_device_components(device)
    _equal(
        mount_device,
        f"{linux_major}:{linux_minor}",

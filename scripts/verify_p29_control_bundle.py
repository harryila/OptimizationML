#!/usr/bin/env python3
"""Fail-closed verification for P29's two offline control-bundle transfers.
...
SCHEMA = "passive-muon-p29-control-bundle-receipt-v1"
STATUS_BY_PHASE = {
...
BRANCH_REF = "refs/heads/p29-permission-safe-bundle-localization-bridge"
...
P29_RECONSTRUCTOR = "scripts/reconstruct_p29_permission_safe_bundle_localization_bridge.py"
P29_CONTRACT = "experiments/training/p29_permission_safe_bundle_localization_bridge_contract.json"
P29_RECONSTRUCTION_SCHEMA = (
    "passive-muon-p29-permission-safe-bundle-localization-bridge-reconstruction-v1"
)
VERIFIER_SOURCE = "scripts/verify_p29_control_bundle.py"
...
NAMESPACE_ROOT = Path("/secure/p29")
SECURE_PARENT = Path("/secure")
TRANSPORT_BASENAME = "transport-20260906-02"
CONTROL_BASENAME = "control-source"
ATTEMPT_BASENAME = "attempt-20260906-02"
EXECUTION_BASENAME = "execution-20260906-02"
BOOTSTRAP_BASENAME = "verify_p29_control_bundle.py"
...
RUNTIME_EVIDENCE_FILES = {
    "runtime_lock": "p29_cuda_runtime_lock.json",
    "host_attestation": "p29_host_attestation.json",
}
RUNTIME_FORBIDDEN_LOCALIZATION_FILES = [
    "p29-run-localization.invoked",
    "p29_cuda_deleted_mapping_localization.native.json",
    "p29_cuda_deleted_mapping_localization.sanitized.json",
    "p29-localization.exit-status.txt",
    "p29-localization-sanitizer.exit-status.txt",
    "p29-p26-failure-ingestion.exit-status.txt",
...
RUNTIME_FORBIDDEN_PREFIXES = [
    "p29-run-contract-reconstruction.",
    "p29-pre-marker-",
    "p29-post-marker-",
    "p29-stage-",
    "p29-pre-ingestion-",
    "p29-p26-failure-ingestion.",
    "p29-pre-localization-",
    "p29-pre-sanitizer-",
    "p29-post-localization-",
    "p29-localization.",
    "p29-localization-sanitizer.",
]
...
SNAPSHOT_SCHEMA = "passive-muon-p29-immutable-execution-snapshot-v1"
...
class VerificationError(RuntimeError):
    """A fail-closed P29 transport-verification error."""
...
class NamespaceGuard:
    """Open-descriptor guard for the permission-safe P29 namespace."""
    ...
    def revalidate(self) -> None:
        ...
        if (
            ...
            raise VerificationError("P29 secure-parent authority changed during verification")
        if _directory_identity(self.namespace_fd, Path(self.namespace.path)) != self.namespace:
            raise VerificationError("P29 namespace authority changed during verification")
        if _directory_identity(self.transport_fd, Path(self.transport.path)) != self.transport:
            raise VerificationError("P29 transport authority changed during verification")
        if (
            _validate_mount_layout(Path(self.namespace.path), Path(self.transport.path))
            != self.mount_check
        ):
            raise VerificationError("P29 namespace mount topology changed during verification")
        if (
            ...
            raise VerificationError("P29 namespace ACL state changed during verification")
    ...
    def bind_namespace_inventory(self, expected: Sequence[str]) -> None:
        ...
        if self.expected_namespace_children is not None:
            raise VerificationError("P29 namespace child inventory was already bound")
        _validate_namespace_child_inventory(
            self.namespace_fd,
            normalized,
            transport=self.transport,
    ...
    def receipt_record(self) -> dict[str, object]:
        if self.expected_namespace_children is None:
            raise VerificationError("P29 namespace child inventory was not bound")
        return {
...
def _validate_namespace_child_inventory(
    ...
    *,
    ...
    """Require an exact directory-only inventory below the P29 namespace."""

    expected_names = tuple(sorted(expected))
    if len(set(expected_names)) != len(expected_names):
        raise VerificationError("P29 namespace child inventory contains duplicates")
    for name in expected_names:
        if name in {"", ".", ".."} or "/" in name:
            raise VerificationError("P29 namespace child inventory contains an unsafe name")
    try:
        actual_names = tuple(sorted(os.listdir(namespace_fd)))
    except OSError as exc:
        raise VerificationError(f"cannot enumerate P29 namespace children: {exc}") from exc
    if actual_names != expected_names:
        raise VerificationError(
            "P29 namespace child inventory differs: "
    ...
    for name in actual_names:
        try:
            ...
            raise VerificationError(f"cannot stat P29 namespace child {name}: {exc}") from exc
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            raise VerificationError(f"P29 namespace child is not a directory: {name}")
        if name == TRANSPORT_BASENAME and (info.st_dev, info.st_ino) != (
            transport.device,
            transport.inode,
        ):
            raise VerificationError("P29 transport namespace entry changed identity")
...
def _validate_mount_layout(namespace: Path, transport: Path) -> dict[str, object]:
    ...
    if str(namespace) in exact_mountpoints or str(transport) in exact_mountpoints:
        raise VerificationError("P29 namespace or transport is an unexpected mountpoint")
    if namespace_mount[:2] != transport_mount[:2]:
        raise VerificationError("P29 namespace and transport cross a mount boundary")
    return {
...
def open_namespace_guard(namespace: Path, transport: Path) -> NamespaceGuard:
    ...
    if not namespace.is_absolute() or not transport.is_absolute():
        raise VerificationError("P29 namespace paths must be absolute")
    if transport != namespace / TRANSPORT_BASENAME:
        raise VerificationError("P29 transport is not the exact namespace child")
    ...
    try:
        ...
        for index, component in enumerate(components):
            if component in {"", ".", ".."} or "/" in component:
                raise VerificationError("P29 namespace has an unsafe path component")
            try:
                child = os.open(component, flags, dir_fd=descriptor)
            except OSError as exc:
                raise VerificationError(
                    f"cannot open P29 directory component {current / component}: {exc}"
                ) from exc
            current /= component
        ...
        if namespace_identity.device != transport_identity.device:
            raise VerificationError("P29 namespace and transport have different devices")
        if namespace_identity.mount_id != transport_identity.mount_id:
            raise VerificationError("P29 namespace and transport have different mount IDs")
        ...
        if (
            ...
            raise VerificationError("P29 secure parent and namespace cross a mount boundary")
        if _is_exact_mountpoint(SECURE_PARENT):
            raise VerificationError("P29 secure parent is an unexpected mountpoint")
...
def verify_immutable_execution_snapshot(
    ...
    *,
    ...
    """Verify the root-owned P29 execution snapshot and its complete manifest."""
...
def authenticate_executing_verifier(
    ...
    if (os.geteuid(), os.getegid()) != (VERIFIER_UID, VERIFIER_GID):
        raise VerificationError(
            "P29 bootstrap verifier execution identity differs from uid/gid "
            f"{VERIFIER_UID}/{VERIFIER_GID}"
        )
    if REQUIRE_EXECUTING_PYTHON_ISOLATION and not (
        sys.flags.isolated == 1 and sys.flags.no_site == 1
    ):
        raise VerificationError("P29 bootstrap Python must run with -I -S")
    ...
    if executing.is_symlink():
        raise VerificationError("executing P29 verifier is a symlink")
    _raw, record = stable_read_nofollow(executing, source_dir_fd=transport_fd)
    if record.sha256 != expected_sha256:
        raise VerificationError("executing P29 verifier bytes differ")
    if record.mode_octal != "0600":
        raise VerificationError("executing P29 bootstrap verifier mode is not 0600")
    if (record.uid, record.gid) != (TRANSPORT_UID, TRANSPORT_GID):
        raise VerificationError("executing P29 bootstrap verifier owner differs")
    if record.link_count != 1:
        raise VerificationError("executing P29 bootstrap verifier is hard linked")
    return record
...
def expected_refs(p29_commit: str) -> dict[str, str]:
    return {
        BRANCH_REF: p29_commit,
...
def assert_phase_history(
    ...
    *,
    ...
    if git_text(repository, ["rev-parse", f"{p29_commit}^{{tree}}"]) != p29_tree:
        raise VerificationError("P29 commit/tree binding differs")
    parents = git_text(repository, ["rev-list", "--parents", "-n", "1", p29_commit]).split()
    if phase == "source":
        if parents != [p29_commit, P28_COMMIT]:
            raise VerificationError("P29 source freeze is not one direct child of terminal P28")
        return {
            "source_commit": p29_commit,
            "source_tree": p29_tree,
    ...
    if parents != [p29_commit, source_commit]:
        raise VerificationError("P29 runtime review is not one direct child of source freeze")
    if git_text(repository, ["rev-parse", f"{source_commit}^{{tree}}"]) != source_tree:
        raise VerificationError("P29 source commit/tree binding differs")
    source_parents = git_text(
        repository, ["rev-list", "--parents", "-n", "1", source_commit]
    ).split()
    if source_parents != [source_commit, P28_COMMIT]:
        raise VerificationError("P29 source freeze history differs")
    delta_raw = run_git(
        [
            "-C",
            str(repository),
...
def create_source_control(
    ...
    *,
    ...
    # Keep HEAD on a distinct unborn name while the closed P29 branch ref is
    ...
    run_git(["init", "--initial-branch=p29-verifier-unborn", str(control)])
    ...
    run_git(["-C", str(control), "checkout", "--detach", p29_commit])
...
def run_frozen_reconstruction(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="p29-python-shim-") as shim_directory:
        python_shim = Path(shim_directory) / "python3"
        python_shim.write_text(
            '#!/bin/sh\nexec "$P29_RECONSTRUCTOR_PYTHON" -I -S "$@"\n',
            encoding="ascii",
        )
        python_shim.chmod(0o700)
        environment = _clean_git_environment()
        environment["PATH"] = f"{shim_directory}:/usr/bin:/bin"
        environment["P29_RECONSTRUCTOR_PYTHON"] = str(Path(sys.executable).resolve())
        process = subprocess.run(
...
def _create_private_reconstruction_checkout(
    ...
    *,
    ...
    run_git(["init", "--initial-branch=p29-private-unborn", str(destination)])
...
def run_required_reconstructions(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="p29-reconstruction-checkout-") as temporary:
        ...
        records = {
            "p29_contract": run_frozen_reconstruction(
                private_control,
                label="P29 contract",
                relative_path=P29_RECONSTRUCTOR,
                expected_source_sha256=None,
                expected_schema=P29_RECONSTRUCTION_SCHEMA,
                expected_canonical_sha256=None,
                expected_check_count=15,
                canonical_artifact_path=P29_CONTRACT,
            ),
    ...
    return {
        ...
        "required_reconstruction_count": 4,
...
def validate_control_checkout(
    ...
    *,
    ...
    if not checked_in_verifier.is_file() or checked_in_verifier.is_symlink():
        raise VerificationError("checked-in P29 verifier is absent or a symlink")
    ...
    if checked_in_record.sha256 != expected_verifier_sha256:
        raise VerificationError("checked-in P29 verifier bytes differ from executing verifier")
...
def validate_layout(args: argparse.Namespace) -> dict[str, Path]:
    ...
    try:
        if NAMESPACE_ROOT.resolve(strict=True) != NAMESPACE_ROOT:
            raise VerificationError("P29 namespace lexical and resolved paths differ")
        if transport.resolve(strict=True) != transport:
            raise VerificationError("P29 transport lexical and resolved paths differ")
    except OSError as exc:
        raise VerificationError(f"cannot resolve P29 permission layout: {exc}") from exc
    expected = {
    ...
    ref_records = [
        {
            ...
            "object_id": object_id,
            "object_type": (
                "commit" if refname in (BRANCH_REF, P28_BRANCH_REF, P27_BRANCH_REF) else "tag"
            ),
            "peeled_commit": {
                P28_TAG_REF: P28_COMMIT,
                P27_TAG_REF: P27_COMMIT,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-p29-commit")
    parser.add_argument("--expected-p29-tree")
...
def _require_normal_cli(args: argparse.Namespace) -> None:
    required = {
        ...
        "expected P29 commit": args.expected_p29_commit,
        "expected P29 tree": args.expected_p29_tree,
    }
...
def verify_execution_snapshot_only(
    ...
    *,
    ...
    namespace_guard.revalidate()
    return {
        "schema_version": "passive-muon-p29-immutable-execution-snapshot-verification-v1",
...
def verify(args: argparse.Namespace, *, namespace_guard: NamespaceGuard) -> dict[str, object]:
    ...
    p29_commit = require_sha1(args.expected_p29_commit, "expected P29 commit")
    p29_tree = require_sha1(args.expected_p29_tree, "expected P29 tree")
    ...
    with tempfile.TemporaryDirectory(prefix="p29-bundle-verification-") as temporary:
    ...
    if not replay:
        if args.phase == "source":
            ...
            advance_runtime_control(
                ...
                source_commit=args.expected_source_commit,
                source_tree=args.expected_source_tree,
                runtime_commit=p29_commit,
                expected_verifier_sha256=expected_verifier_sha256,
            )
    reconstruction = validate_control_checkout(
        paths["control"],
        expected_head=p29_commit,
        expected_tree=p29_tree,
        refs=refs,
        run_reconstruction=True,
        expected_verifier_sha256=expected_verifier_sha256,
    )
    ...
    if args.phase == "source":
        ...
        if (
            validate_runtime_attempt(paths["attempt"], control=paths["control"])
            != final_attempt_guard
        ):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"P29 control-bundle verification failed: {exc}", file=sys.stderr)
        return 1
    finally:

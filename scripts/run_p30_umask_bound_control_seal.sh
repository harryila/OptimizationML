...
lock_raw = stable_bytes(lock_path, 1000, 1000, 0o600, expected_lock_sha256)
attestation_raw = stable_bytes(
    attestation_path, 1000, 1000, 0o600, expected_attestation_sha256
)
...
set_native_private_mode_for_p27_sanitizer() {
  ...
  /usr/bin/python3 -I -S - \
    "$path" "$P30_HOST_EVIDENCE" "$expected_sha256" "$expected_byte_count" \
...
path = pathlib.PurePosixPath(sys.argv[1])
evidence = pathlib.PurePosixPath(sys.argv[2])
expected_sha256 = sys.argv[3]
expected_byte_count = int(sys.argv[4])
expected_device = int(sys.argv[5])
expected_inode = int(sys.argv[6])
if path.parent != evidence or path.name in {"", ".", ".."}:
    raise SystemExit("P30 sanitizer native is outside the evidence root")
directory_flags = os.O_RDONLY | os.O_CLOEXEC | os.O_DIRECTORY | os.O_NOFOLLOW
parent_fd = os.open("/", directory_flags)
descriptor = None
try:
    ...
    if (
        ...
        raise SystemExit("P30 sanitizer evidence-root authority differs")
    descriptor = os.open(
        path.name,
        os.O_RDWR | os.O_CLOEXEC | os.O_NOFOLLOW,
        dir_fd=parent_fd,
    ...
    if (
        ...
        raise SystemExit("P30 sealed sanitizer-native authority differs")
    forbidden_acl = {
        "system.posix_acl_access",
        "system.posix_acl_default",
        "system.nfs4_acl",
    ...
    for target, label in ((parent_fd, "parent"), (descriptor, "native")):
        acl_names = {
            value.decode() if isinstance(value, bytes) else value
            for value in os.listxattr(target)
        }
        if acl_names & forbidden_acl:
            raise SystemExit(f"P30 sanitizer {label} has a forbidden ACL")
    ...
    if digest_fd(descriptor) != expected_sha256:
        raise SystemExit("P30 sealed sanitizer-native digest differs")
    ...
    if stable(after) != stable(before) or stable(after) != stable(by_name):
        raise SystemExit("P30 sanitizer-native identity changed during mode bridge")
    if stat.S_IMODE(after.st_mode) != 0o600:
        raise SystemExit("P30 sanitizer-native private mode was not established")
    if digest_fd(descriptor) != expected_sha256:
        raise SystemExit("P30 sanitizer-native bytes changed during mode bridge")
    os.fsync(parent_fd)
finally:
    if descriptor is not None:
        os.close(descriptor)
...
sanitize_localization_artifact() {
  ...
  shift 4
  local binding native_sha256 native_byte_count native_device native_inode
  local native_uid native_gid native_mode extra
  ...
  IFS=: read -r native_sha256 native_byte_count native_device native_inode \
    native_uid native_gid native_mode extra <<<"$binding"
  [[ "$native_sha256" =~ ^[0-9a-f]{64}$ && \
     "$native_byte_count" =~ ^[1-9][0-9]*$ && \
     "$native_device" =~ ^[0-9]+$ && "$native_inode" =~ ^[0-9]+$ && \
     "$native_uid" == 0 && "$native_gid" == 0 && \
     "$native_mode" == 0444 && -z "$extra" ]] ||
    die "P30 native localization binding is malformed"
  printf 'native_binding=%s\n' "$binding"
  set_native_private_mode_for_p27_sanitizer \
    "$native" "$native_sha256" "$native_byte_count" \
    "$native_device" "$native_inode" ||
    die "P30 native localization private-mode bridge failed"
  local sanitizer_status=0 reseal_status=0 rebound
  set +e
  p30_docker exec \
    --env "CUDA_VISIBLE_DEVICES=$P30_GPU_UUID" \
    --env "NVIDIA_VISIBLE_DEVICES=$P30_GPU_UUID" \
    ...
    "$@"
  sanitizer_status=$?
  seal_retained_file "$native"
  reseal_status=$?
  set -e
  (( reseal_status == 0 )) ||
    die "P30 native localization artifact did not reseal after sanitization"
  rebound="$(validate_root_evidence_file "$native" - - 0444 0 0)" ||
    die "P30 resealed native localization artifact could not be rebound"
  [[ "$rebound" == "$binding" ]] ||
    die "P30 native localization identity changed across sanitization"
  (( sanitizer_status == 0 )) || return "$sanitizer_status"
